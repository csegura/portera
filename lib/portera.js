...
const portera = (cfg) => {
  ...
  const portera_fncs = [
    ...
    { name: "logobj", fnc: portera_logobj },
  ];
  ...
  try {
    ...
    cfg.options = cfg.options || portera_fncs.map((e) => e.name); // ["log", "info", "warn", "error", "assert", "trace", "btrace", "stack", "logobj"];
    cfg.hide = cfg.hide || [];
    setup();
