...
const portera = (cfg) => {
  ...
  const portera_dump = (...args) => {
    ...
    socket.emit("log", log_msg("dump", sargs));
  };
  ...
  try {
    ...
    cfg.use = cfg.use || portera_fncs.map((e) => e.name); // ["log", "info", "warn", "error", "assert", "trace", "btrace", "stack", "dump"];
    cfg.nouse = cfg.nouse || [];
    setup();
